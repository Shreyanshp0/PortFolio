...
export function Projects() {
  ...
  const filtered = useMemo(() => {
    if (normalizeCategory(activeFilter) === 'all') return projects

    return projects.filter((project) => normalizeCategory(project.category) === normalizeCategory(activeFilter))
  }, [activeFilter])
          ...
          {projectFilters.map((filter, index) => (
            <motion.button
              ...
              className={`neo-btn px-4 py-2 text-sm font-semibold ${
                activeFilter === filter ? highlightAccents[index % highlightAccents.length] : 'bg-theme-card text-theme-primary'
              }`}
              {...mechanicalButton}
            >
              {filter}
        ...
        <div className="grid gap-8 xl:grid-cols-[1.3fr,0.7fr]">
          ...
          >
            ...
            {filtered.length ? (
              ...
              <div
                className="neo-card col-span-full p-8 text-center"
                style={{ borderRadius: '18px 8px 18px 8px' }}
              >
                <p className="text-lg font-black">No projects in this category yet.</p>
